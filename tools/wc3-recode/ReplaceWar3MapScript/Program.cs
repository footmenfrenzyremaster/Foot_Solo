...
var scriptPath = args.Length > 1 ? Path.GetFullPath(args[1]) : Path.Combine(root, "outputs", "WC3-799", "build", "war3map.cleaned.candidate.j");
var outputMapPath = args.Length > 2 ? Path.GetFullPath(args[2]) : Path.Combine(root, "outputs", "WC3-799", "releases", "799W-tester-cleaned-jass.w3x");
var internalName = args.Length > 3 ? args[3] : "war3map.j";
...
PatchMpqFile(outputMapPath, internalName, scriptBytes);

Console.WriteLine($"Wrote map:       {outputMapPath}");
Console.WriteLine($"Internal file:   {internalName}");
Console.WriteLine($"Payload bytes:   {scriptBytes.Length:n0}");
Console.WriteLine($"Payload SHA256:  {Convert.ToHexString(SHA256.HashData(scriptBytes)).ToLowerInvariant()}");
